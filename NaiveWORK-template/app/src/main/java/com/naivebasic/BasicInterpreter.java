...
public class BasicInterpreter {
    ...
    public String run(String source) {
        ...
        for (int pc = 0; pc < program.size(); pc++) {
            ...
            try {
                ...
                if (u.equals("END")) { if (input instanceof ScreenProvider) { ((ScreenProvider)input).stopPlay(); ((ScreenProvider)input).flushDisplay(); } break; }
    ...
    private Integer executeInline(String t, int pc) throws Exception {
        String u = t.toUpperCase(Locale.US);
        if (u.equals("END")) { if (input instanceof ScreenProvider) { ((ScreenProvider)input).stopPlay(); ((ScreenProvider)input).flushDisplay(); } return Integer.valueOf(program.size()); }
    ...
    private class NumExpr {
        ...
        double factor()throws Exception{
            ...
            if(p<s.length()&&Character.isLetter(s.charAt(p))){
                ...
                if(n.equals("PLAY")) return ((ScreenProvider)input).isPlayPlaying() ? 1.0 : 0.0;
